...
-- Nro. SC único entre pedidos activos. Normaliza valores como "SC001307",
-- "001307" y "1307" al mismo identificador para evitar duplicados por formato.
...
-- ----------------------------------------------------------------------------
-- 9. Correcciones trazables del historial de estados de los ítems
-- ----------------------------------------------------------------------------

alter table detalle_historial_estados
...
begin
  if not exists (
    ...
    alter table detalle_historial_estados
      ...
      references detalle_historial_estados(historial_id);
  end if;
end $$;

create table if not exists detalle_historial_auditoria (
  auditoria_id bigserial primary key,
  historial_id bigint not null references detalle_historial_estados(historial_id),
...
alter table detalle_historial_auditoria enable row level security;
drop policy if exists authenticated_select on detalle_historial_auditoria;
create policy authenticated_select on detalle_historial_auditoria
  for select to authenticated using (true);
grant select on detalle_historial_auditoria to authenticated;

create or replace function fn_editar_comentario_movimiento_detalle(
...
create or replace function fn_corregir_ultimo_movimiento_detalle(
  p_detalle_id bigint,
  p_historial_id bigint,
  p_estado_id int,
  p_motivo text
...
revoke all on function fn_editar_comentario_movimiento_detalle(bigint, text) from public;
revoke all on function fn_corregir_ultimo_movimiento_detalle(bigint, bigint, int, text) from public;
grant execute on function fn_editar_comentario_movimiento_detalle(bigint, text) to authenticated;
grant execute on function fn_corregir_ultimo_movimiento_detalle(bigint, bigint, int, text) to authenticated;

-- Cambio masivo, atómico y trazable de un subconjunto de ítems del pedido.
...
begin
  ...
  from jsonb_to_recordset(p_detalles) as entrada(detalle_id bigint, cantidad_aprobada numeric);
  ...
  if exists (
    select 1 from detalle_pedido d join estados_catalogo ec on ec.estado_id = d.estado_actual_id
    where d.detalle_id = any(v_detalle_ids) and ec.nombre = 'Atendido'
  ) then
    raise exception 'No se puede cambiar masivamente un ítem Atendido';
  end if;
  if exists (
    select 1 from detalle_pedido where detalle_id = any(v_detalle_ids) and estado_actual_id = p_estado_id
  ) then
    raise exception 'Quita de la selección los ítems que ya tienen el estado elegido';
  end if;
  if v_estado_nombre = 'Aprobado' and exists (
    select 1 from jsonb_to_recordset(p_detalles) as entrada(detalle_id bigint, cantidad_aprobada numeric)
  ...
  if v_estado_nombre = 'En compra' and exists (
    ...
    where d.detalle_id = any(v_detalle_ids) and not exists (
      select 1 from detalle_historial_estados h join estados_catalogo ec on ec.estado_id = h.estado_id
      where h.detalle_id = d.detalle_id and ec.nombre = 'Aprobado'
    )
  ) then
  ...
  set estado_actual_id = p_estado_id,
      cantidad_aprobada = case when v_estado_nombre = 'Aprobado' then entrada.cantidad_aprobada else d.cantidad_aprobada end
  from jsonb_to_recordset(p_detalles) as entrada(detalle_id bigint, cantidad_aprobada numeric)
  where d.detalle_id = entrada.detalle_id;

  insert into detalle_historial_estados (detalle_id, estado_id, fecha, comentario)
  select entrada.detalle_id, p_estado_id, coalesce(p_fecha, now()), nullif(btrim(p_comentario), '')
  from jsonb_to_recordset(p_detalles) as entrada(detalle_id bigint, cantidad_aprobada numeric);
